...
export default function ImportGoogleFormPage() {
  ...
  function buildSoalFromRow(row: string[], idx: number) {
    ...
    const opsiArr = tipe === 'pg'
      ? opsiRaw.map((o, i) => o ? `${['A','B','C','D'][i]}. ${o}` : null).filter(Boolean) as string[]
      : null
  ...
  async function handleImport() {
    ...
    const pertanyaanIdx = mapping['pertanyaan']
    const tipeIdx = mapping['tipe']
    if (pertanyaanIdx === undefined || pertanyaanIdx < 0) { alert('Kolom "Pertanyaan" wajib dipetakan.'); return }
    if (tipeIdx === undefined || tipeIdx < 0) { alert('Kolom "Tipe Soal" wajib dipetakan.'); return }
      ...
      <details className="card bg-amber-50 border-amber-200">
        ...
        <div className="mt-3 space-y-2">
          <p className="text-xs text-amber-700">
            ...
            Setiap baris jawaban dosen akan menjadi satu soal.
          </p>
          ...
          <p className="text-xs text-amber-600">
            Setelah dosen mengisi form, buka <strong>tab Responses</strong> di Google Form → klik ikon Sheets hijau untuk membuat spreadsheet jawaban →
            di Sheets, klik <strong>File → Share → Akses umum → Siapa saja yang memiliki link (Viewer)</strong> → copy link sheet tersebut.
          </p>
        </div>
      </details>
      ...
      {step === 1 && (
        <>
          ...
          <div className="card space-y-3">
            ...
            <button onClick={handleAmbilData} disabled={fetching || !selectedUjian} className="btn-primary w-full">
              {fetching ? (
            ...
            {!selectedUjian && <p className="text-xs text-amber-600 text-center">Pilih ujian tujuan terlebih dahulu.</p>}
          </div>
        </>
      )}
        ...
        <>
          <div className="card bg-green-50 border-green-200">
            <p className="text-sm text-green-700">
              ✅ Ditemukan <strong>{rows.length} baris data</strong> dengan <strong>{headers.length} kolom</strong>. Cocokkan kolom di bawah ini.
            </p>
          </div>
      ...
      {step === 3 && result && (
        <>
          ...
          <div className="flex gap-3 pb-4">
            ...
            {selectedUjian && (
              <a href={`/admin/ujian/${selectedUjian}`} className="btn-primary flex-1 text-center">
                Lihat & Edit Soal →
              </a>
            )}
          </div>
